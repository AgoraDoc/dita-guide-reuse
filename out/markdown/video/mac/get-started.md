# Get Started with Video Call for macOS {#get-started-with-product-name-for-platform}
...
2.  Integrate the Agora Video SDK into your project:
    ...
    3.  Open the \[/dita/topic/topic/body/ol/li/ol/li/codeph \{"- topic/codeph "\}\)Podfile\(codeph\], and replace all contents with the following code. Remember to replace \[/dita/topic/topic/body/ol/li/ol/li/codeph \{"- topic/codeph "\}\)Your App\(codeph\] with the target name of your project.
...
1.  Replace the contents in the `ViewController.h` file with the following:
    ...
    \[/dita/topic/topic/topic/body/topic/body/ol/li/p/codeblock \{"- topic/codeblock "\}\) \#import <AppKit/AppKit.h\> \#import <AgoraRtcKit/AgoraRtcEngineKit.h\> @interface ViewController : NSViewController <AgoraRtcEngineDelegate\> @property \(strong, nonatomic\) AgoraRtcEngineKit \*agoraKit; @end \(codeblock\]
...
3.  Open the `Podfile`, delete all contents, and input the following codes. Remember to replace `Your App` with the target name of your project and replace `version` with the version of the SDK that you want to integrate. For information about the SDK version, see [\[/dita/topic/topic/topic/body/ol/li/p/ph/p/codeblock \{"- topic/codeblock "\}\) \# platform :target 'Your App' do pod '', 'version' end\(codeblock\]

](https://docs.agora.io/en/Video/release_mac_video?platform=macOS)9.  Return to Terminal, and run the `pod install` command to install the Agora SDK. Once you successfully install the SDK, it shows `Pod installation complete!` in Terminal, and you can see an `xcworkspace` file in the project folder.
